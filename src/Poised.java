 ...
 * by sending their respective file name.
 * <br>
...
public class Poised {
	public static void main(String[] args) {
		Contacts customerContacts, contractorContacts, architectContacts;
		...
		customerContacts = new Contacts("customerContacts.txt");
		customerContacts.setContactList();
		contractorContacts = new Contacts("contractorContacts.txt");
		contractorContacts.setContactList();
		architectContacts = new Contacts("architectContacts.txt");
		architectContacts.setContactList();
		
		// Creating an object under 'ProjectDetails' and generating a list of 'Project' objects.
		readProj = new ProjectDetails(customerContacts.getContactList(), contractorContacts.getContactList()
				, architectContacts.getContactList());
		...
		while (true) {
			...
			if (choice.equals("a")) {
				addProj(readProj, user, customerContacts, contractorContacts, architectContacts);
			...
			// Accessing methods under 'readProj' object to write to files.
				...
				readProj.writeProjFile();
				readProj.writeCompProjFile();
				readProj.writeContactFiles("customerContacts.txt"
						, customerContacts.getContactList());
				readProj.writeContactFiles("contractorContacts.txt"
						, contractorContacts.getContactList());
				readProj.writeContactFiles("architectContacts.txt"
						, architectContacts.getContactList());
				break;
			}else {
	 ...
	 * @param cust 'Contacts' object of all customers.
	 * @param cont 'Contacts' object of all contractors.
	 * @param arch 'Contacts' object of all architect.
	 */
	public static void addProj(ProjectDetails readProj, Scanner input
			, Contacts cust, Contacts cont, Contacts arch) {
		...
		cust.addToContactList(obj[0]);
		cont.addToContactList(obj[1]);
		arch.addToContactList(obj[2]);
		...
		readProj.addToList( new Project(projName, projNum, projAddress
